...
.. module:: django_msgspec.serializers

msgspec-powered copies of Django’s built-in JSON serializers, for use with |dumpdata|__ and |loaddata|__.
...
The serializers’ output is compatible with Django’s standard JSON deserializer, with one textual difference: ``DurationField`` values are written in msgspec’s compact ISO 8601 form, e.g. ``"P1DT7200S"``, rather than Django’s expanded form, e.g. ``"P1DT02H00M00S"``.
Both forms are parsed correctly by Django’s deserializers, so fixtures remain interchangeable in both directions.
