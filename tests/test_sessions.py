...
from django_msgspec.sessions import JSONSerializer
...
class SerializerCompatibilityTests(TestCase):
    def test_django_json_encoded_decoded_by_msgspec(self):
        data = {"_auth_user_id": "42", "key": "value"}
        encoded = DjangoJSONSerializer().dumps(data)
        assert JSONSerializer().loads(encoded) == data

    def test_msgspec_encoded_decoded_by_django_json(self):
        data = {"_auth_user_id": "42", "key": "value"}
        encoded = JSONSerializer().dumps(data)
        assert DjangoJSONSerializer().loads(encoded) == data

    def test_django_json_encoded_decoded_by_msgspec_non_ascii(self):
        data = {"username": "héllo", "city": "東京"}
        encoded = DjangoJSONSerializer().dumps(data)
        assert JSONSerializer().loads(encoded) == data

    @pytest.mark.xfail
    def test_msgspec_encoded_decoded_by_django_json_non_ascii(self):
        """
        Sessions written by django-msgspec's JSONSerializer are silently
        corrupted when read back by Django's serializer, which decodes the
        payload as latin-1 before parsing.
        """
        data = {"username": "héllo", "city": "東京"}
        encoded = JSONSerializer().dumps(data)
        decoded = DjangoJSONSerializer().loads(encoded)
        assert decoded == data, (
            f"Session written by django-msgspec's JSONSerializer was read back by Django's"
