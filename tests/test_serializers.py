...
from django_msgspec.serializers.json import Deserializer as JsonDeserializer
from django_msgspec.serializers.json import Serializer as JsonSerializer
from django_msgspec.serializers.jsonl import Deserializer as JsonlDeserializer
from django_msgspec.serializers.jsonl import Serializer as JsonlSerializer
...
class JsonSerializerTests(TestCase):
    ...
    def test_indent(self):
        ...
        msgspec.json.decode(result)

    def test_indent_4(self):
        w = Widget.objects.create(name="test")
        result = JsonSerializer().serialize(Widget.objects.all(), indent=4)
        assert (
            result
            == f'[\n{{\n    "model": "testapp.widget",\n    "pk": {w.pk},\n    "fields": {{\n        "name": "test",\n        "price": null,\n        "created_at": null,\n        "duration": null\n    }}\n}}\n]\n'
        )
    ...
    def test_output_to_stream(self):
        ...
        data = msgspec.json.decode(stream.getvalue())
        assert len(data) == 1
...
class JsonlSerializerTests(TestCase):
    def test_basic_structure(self):
        ...
        assert len(lines) == 1
        data = msgspec.json.decode(lines[0])
    ...
    def test_multiple_objects(self):
        ...
        lines = [line for line in result.splitlines() if line]
        assert len(lines) == 2
        assert msgspec.json.decode(lines[0])["fields"]["name"] in ("a", "b")
        assert msgspec.json.decode(lines[1])["fields"]["name"] in ("a", "b")

    def test_decimal_field(self):
        Widget.objects.create(name="test", price=decimal.Decimal("9.99"))
        result = JsonlSerializer().serialize(Widget.objects.all())
        data = msgspec.json.decode(result.splitlines()[0])
