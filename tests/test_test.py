...
from django_msgspec.test import AsyncClient, Client, SimpleTestCase
...
class ClientTests(DjangoSimpleTestCase):
    ...
    def test_encode_json_uses_django_msgspec_default(self):
        assert (
            self.client._encode_json(  # type: ignore[attr-defined]
                {"duration": datetime.timedelta(days=1, hours=2)}, "application/json"
            )
            == b'{"duration":"P1DT7200S"}'
        )
    ...
    def test_parse_json_extra_error(self):
        ...
        assert (
            str(excinfo.value)
            == "msgspec.json.decode() does not accept keyword arguments"
        )
