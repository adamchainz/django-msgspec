...
class JsonScriptFilterTests(SimpleTestCase):
    def test_basic(self):
        template = Template("{% load django_msgspec %}{{ data|json_script }}")
        rendered = template.render(Context({"data": {"key": "value"}}))
        assert rendered == '<script type="application/json">{"key":"value"}</script>'

    def test_element_id(self):
        template = Template("{% load django_msgspec %}{{ data|json_script:'data' }}")
        rendered = template.render(Context({"data": {"key": "value"}}))
        assert rendered == (
    ...
    def test_complex_escaping(self):
        template = Template(
            "{% load django_msgspec %}{{ value|json_script:'test_id' }}"
        )
